...
This project processes multi-sensor data from a vehicle equipped with a top-mounted lidar, GNSS-INS system, and multiple cameras. The goal is to create a comprehensive static point cloud representation of the environment by aggregating lidar frames, filtering moving objects, and colorizing the result using camera imagery.

All sensors are intrinsically and extrinsically calibrated, time-synchronized, and supported by RTK correction data for GNSS accuracy. The implementation uses the nuScenes dataset and processes data without manual annotations.
...
GNSS_Solomon/
├── scripts/
│   ├── aggregate_task1.py    # Task 1: Point cloud aggregation
│   ├── task2.py               # Task 2: Moving object filtering
│   └── task3.py               # Task 3: Point cloud colorization
├── outputs/                   # Generated point cloud files
...
For each frame:
┌──────────────┐
│ Lidar Frame  │  (Sensor coordinate system)
│   Points     │
└──────┬───────┘
       │
       ...
       ▼
┌─────────────────────────┐
│ Build T_sensor_to_ego  │  R(q_sensor), t_sensor
│ (4x4 transformation)    │
└──────────┬──────────────┘
           │
           ▼
┌─────────────────────────┐
│ Build T_ego_to_global   │  R(q_ego), t_ego (GNSS-INS)
│ (4x4 transformation)    │
└──────────┬──────────────┘
           │
           ▼
┌─────────────────────────┐
│ Apply Transformation    │  P_global = T_ego_to_global @
│ to all points           │              T_sensor_to_ego @ P_sensor
└──────────┬──────────────┘
           │
           ▼
┌─────────────────────────┐
│ Accumulate in Global   │  Concatenate all transformed points
│ Point Cloud             │
└──────────┬──────────────┘
           │
           ▼
┌─────────────────────────┐
│ Save PCD File           │  outputs/taask1_scene-XXX.pcd
└─────────────────────────┘
```
...
Identify and remove data corresponding to moving objects from the aggregated point cloud to create a static representation of the environment.

### Approach

The filtering pipeline uses geometric analysis without temporal tracking:

1. **Ground Removal**: Estimate local ground height using percentile-based method
2. **Height Band Filtering**: Identify points in vehicle height range (0.25m to 3.2m above ground)
3. **2D Clustering**: Cluster height-band points using DBSCAN
4. **Geometric Filtering**: Reject wall-like structures and keep vehicle-like clusters based on dimensions
...
┌──────────────────────────┐
│ Aggregated Point Cloud   │  (from Task 1)
│   P = [x, y, z]          │
└───────────┬──────────────┘
            │
            ▼
...
1. Divide XY plane into cells of size `cell_size` (default 0.4m):
```
...
Default percentile is 12.0, which selects points near the ground surface while rejecting outliers.
...
```
above_ground = h > h_min  (default: 0.25m)
vehicle_band = (h > h_min) & (h < h_max)  (default: 0.25m - 3.2m)
```
...
2. Apply DBSCAN clustering:
   - `eps`: Maximum distance between points in same cluster (default: 0.65m)
   - `min_points`: Minimum points required to form cluster (default: 8)
...
2. **Vehicle-like Filtering Criteria**:

A cluster is kept if it satisfies:
- Height constraint: `h_min <= height <= min(h_max, veh_max_h)`
- Length constraint: `length_xy <= veh_max_len` (default: 15.0m)
- Width constraint: `width_xy <= veh_max_w` (default: 5.0m)
- Minimum points: `cluster_size >= veh_min_points` (default: 25)
...
- **No temporal tracking required**: Geometric analysis works on single aggregated cloud, avoiding complex multi-frame tracking
- **Percentile-based ground estimation**: Robust to outliers and varying terrain, more reliable than mean or median
- **Height band filtering**: Focuses analysis on vehicle-height objects, reducing computational load
- **2D clustering**: Vehicles appear as connected regions in top-down view, clustering in XY plane is sufficient
- **Geometric heuristics**: Vehicle dimensions are well-constrained (length, width, height), allowing reliable identification
- **Wall rejection**: Buildings and facades have distinct geometric signatures (long, thin, tall) that differ from vehicles
...
┌──────────────────────────┐
│ Static Point Cloud       │  (from Task 2, moving objects removed)
│   P_world = [x, y, z]    │
└───────────┬──────────────┘
            │
            ▼
...
│                                          │
│ Points with RGB colors from nearest      │
│ visible camera                           │
└──────────────────────────────────────────┘
```
...
### Why 2D Clustering Instead of 3D?
...
### Why Geometric Heuristics Instead of Machine Learning?

Geometric constraints (vehicle dimensions) are well-defined and reliable. Heuristics are interpretable, require no training data, and work across different vehicle types. Machine learning would require labeled data and may overfit to specific scenarios.
...
1. **Static assumption**: Method assumes moving objects can be identified geometrically without temporal analysis
2. **Parameter sensitivity**: Filtering thresholds are tuned for urban driving scenarios
3. **Occlusion handling**: Points occluded in all cameras remain uncolored
4. **Single frame colorization**: Uses cameras from first frame only, not temporal fusion
